...
export type NavigationConfigType = (p: ParamsType) => {
    /** The initial group to be loaded by the router */
    initialGroup: string
    groups: GroupConfigType
}

export type NavigationDefConfigType = {
    initialGroup?: string
    groups: {
        [key: string]: {
            condition?: boolean
...
export type NavigatorTypes = 'tabs' | 'stack' | string

/**
 * Defines the configuration that should be passed to the make config
 * @author Alejandro Quiroz <alejandro.devop@gmail.com>
 */
export type MakeNavHookReturnType = (p?: ParamsType) => {
 ...
 *
 */
export type UseNavigateReturnType = {
    state: {
        key: string
        index: number
        routeNames: string[]
    ...
    goTo: (p: string, state?: { [key: string]: string | boolean | number }) => void
    /** Function to go back */
    goBack: () => void
}

export type TabOptionsTypes = {
